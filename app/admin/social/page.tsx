...
const PRESET_ICONS: Record<string, { svgPath: string; hoverColor: string; defaultUrl?: string; position: 'top' | 'right' | 'both' }> = {
  Phone: {
    hoverColor: '#00FFE1',
...
export default function AdminSocialPage() {
  ...
  const [activeTab, setActiveTab] = useState<'all' | 'top' | 'right'>('all');
  ...
  const filteredLinks = links.filter(l => {
    if (activeTab === 'top') return l.position === 'top' || l.position === 'both';
    if (activeTab === 'right') return l.position === 'right' || l.position === 'both' || !l.position;
    return true;
  });
        ...
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          <div className="flex items-center gap-2 border-b border-white/10 pb-3 overflow-x-auto">
            <button
              ...
              className={`px-3.5 py-1.5 rounded-lg text-xs sm:text-sm font-medium transition-colors ${activeTab === 'all' ? 'bg-white/15 text-white border border-white/20' : 'text-gray-400 hover:text-white'}`}
            >
            ...
            >
              Top Navbar ({links.filter(l => l.position === 'top' || l.position === 'both').length})
            </button>
            <button
            ...
            >
              Right Side Dock ({links.filter(l => l.position === 'right' || l.position === 'both' || !l.position).length})
            </button>
          </div>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`text-[10px] px-2 py-0.5 rounded-md border font-mono ${
                              ...
                              {pos === 'top' ? 'Top Navbar' : pos === 'both' ? 'Top & Right' : 'Right Side'}
                            </span>
                          </div>
            ...
            <div className="bg-[#111] border border-white/10 rounded-2xl p-5 sm:p-6 w-full max-w-lg space-y-4 max-h-[90vh] overflow-y-auto">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => setEditing(p => p ? { ...p, position: 'both' } : p)}
                    className={`py-2 px-3 rounded-xl border text-xs font-medium transition-colors ${
                      editing.position === 'both'
