...
export default function ProjectGalleryGrid({
  ...
  return (
    <>
      {/* BANNER MODE (Static Banner Image - Non-clickable) */}
      {onlyBanner && mainImage && (
        <div
          className="max-w-3xl sm:max-w-4xl mx-auto rounded-2xl overflow-hidden mb-10 shadow-2xl bg-[#0d0d0d]/80 p-3 sm:p-5 flex items-center justify-center border border-white/10"
        ...
        >
          ...
          <img
            src={mainImage}
            alt={projectName}
            className="w-auto max-w-full h-auto max-h-[380px] sm:max-h-[460px] object-contain rounded-lg drop-shadow-xl"
          />
        </div>
      )}

      {/* GRID MODE (Rendered inside left column below How it Works) */}
      {onlyGrid && (
        <section className="mt-8">
          <h2 className="text-xl font-bold mb-4 text-white border-b border-white/10 pb-2 flex items-center justify-between">
            <span>Gallery</span>
            <span className="text-xs text-gray-400 font-normal">({allImages.length} images)</span>
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {allImages.map((imgUrl: string, i: number) => (
              <div
                ...
                className="group relative rounded-xl overflow-hidden border border-white/10 bg-[#0d0d0d] hover:border-white/40 transition-all duration-300 cursor-pointer h-48 sm:h-52 flex items-center justify-center p-2"
              >
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={imgUrl}
                ...
                <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-2">
                  <span className="bg-black/80 text-white text-xs font-semibold px-3 py-1.5 rounded-full border border-white/20 flex items-center gap-1.5 backdrop-blur-sm">
                ...
                {i === 0 && mainImage && (
                  <span className="absolute top-2 left-2 bg-gradient-to-r from-pink-500 to-orange-500 text-white text-[10px] font-bold px-2 py-0.5 rounded shadow">
                    Main Banner
                  </span>
                )}
              </div>
      ...
      {/* FULL-SCREEN LIGHTBOX MODAL CAROUSEL */}
      <AnimatePresence>
        {isOpen && (
          <motion.div
            initial={{ opacity: 0 }}
              ...
              <div className="w-full max-w-screen-md flex items-center justify-center gap-3 overflow-x-auto py-2 px-4 z-10 scrollbar-thin">
                {allImages.map((imgUrl, i) => (
                  <button
                    ...
                    className={`relative w-16 h-12 sm:w-20 sm:h-14 rounded-lg overflow-hidden border-2 transition-all flex-shrink-0 bg-[#1a1a1a] ${i === activeIndex
                        ? 'scale-110 shadow-lg'
