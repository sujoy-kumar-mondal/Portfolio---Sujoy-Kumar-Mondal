...
export default function ProjectGalleryManager({
  ...
  // Build deduplicated image list (mainImage first)
  ...
  return (
    <>
      ...
      {/* ─── 2. FULL-SCREEN LIGHTBOX MODAL CAROUSEL ─── */}
      <AnimatePresence>
        {isOpen && (
          <motion.div
            initial={{ opacity: 0 }}
      ...
      {/* Expose Gallery Grid Section to render inside left column */}
      {allImages.length > 0 && (
        <span data-gallery-grid-anchor="" className="hidden" />
      )}
    </>
  );
}
